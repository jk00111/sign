...
import com.example.sign.step.entity.ReviewLine;
...
public class Sign {
    ...
    public SignResult approve(Submit submit) {
        ...
        ApprovalResult result = approvalService.approve(submit);
    ...
    public SignResult rejectApproval(Submit submit) {
        long signId = submit.getSignId();
        ApprovalResult result = approvalService.reject(submit);
        signService.update(signId, result);

        return SignResult.rejected(signId);
    }

    public SignResult review(long id, ApprovalUser user) {
        ReviewLine reviewLine = new ReviewLine(id, stepService.findByReview(id));
        ReviewEvent event = stepService.review(reviewLine, user);

        reviewService.review(id, event);
        return new SignResult(id, event.isReviewed());
    }

    public SignResult rejectReview(long id, ApprovalUser user) {
        ReviewLine reviewLine = new ReviewLine(id, stepService.findByReview(id));
        ApprovalEvent event = stepService.review(reviewLine, user);

        reviewService.reject(id, event);
        return new SignResult(id, event.isRejected());
    }
}
